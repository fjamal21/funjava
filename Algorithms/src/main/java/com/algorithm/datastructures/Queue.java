...
public class Queue {
	 ...
	 * 
	 * @param size
	 *            the size to set the new queue to.
	 */
	Queue(int size) {
		this.size = size;
	...
	public void remove() {
		if (numberOfItems > 0) { // If the queue contains items
			System.out.println("Item \"" + contents[front]
					+ "\" was removed from the queue.");
			for (int i = 0; i < contents.length - 1; i++) { // Shift everything
															// in the queue left
	 ...
	 * 
	 * @param o
	 *            the array of objects to be added to the queue.
	 */
	 ...
	 * 
	 * @param i
	 *            the column number to calculate for
	 ...
	 * 
	 * @param type
	 *            specifies whether to print the indices or the data.
	 */
		...
		for (int i = 0; i < contents.length; i++) {
			if (type == "index") // Print index row
				...
				if (contents[i] != null) // If data exists
					...
					// If data doesn't exist, print blank space
					System.out.printf("| %-" + getColumnWidth(i) + "s ", "");
	...
	/**
	 * Displays the contents of the queue in a table format. This is a terrible
	 * way to output the table I'm sure, but this is not the purpose of the
	 * class.
	 */
	public void display() {
		int i = 0;
